    ...
    def __call__(self, value):
        if value.size > self.max_size:
            raise serializers.ValidationError(
                _('Size too large, the file has {} and only {} is allowed'.format(
...
class ExtensionValidator(object):
    ...
    def __init__(self, allowed_extension=("doc", "pdf", "jpg", "png", "gif", "xlsx")):
    ...
    def __call__(self, value):
        ...
        if ext.lower() not in self.allowed_extension:
            raise serializers.ValidationError(
                _('File extension not allowed, the file has {} '
                  'extension and only {} are allowed'.format(ext, ", ".join(self.allowed_extension)))
            )
